...
export async function generateCharts({
  ...
  runs.forEach((run, runIndex) => {
    ...
    run.Suites.forEach(suite => {
      for (let bench of suite.Benchmarks) {
        let configCharName;
        if (perBenchmark){
        ...
        if (charts[makeChartName(configCharName, chartsTypes[0])]) {
          ...
          // In the event that we're adding graphs per package and not per benchmark we need to break so we're not
          // creating a new chart for each benchmark.
          if(!perBenchmark){
            break;
          }
        }
